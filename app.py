...
DETECTION_DIR = PROJECT_ROOT / "vision" / "Artifacts"
...
def generate_image_endpoint(data: ImageInput):
    from vision.src.image_generation import ImageGenerator
...
async def detect_object_endpoint(image: UploadFile = File(...)):
    ...
    # 3. run the detector
    try:
        detector = FashionDetector(PROJECT_ROOT / "vision" / "model" / "best.pt")
